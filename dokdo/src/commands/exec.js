...
export async function exec(message, parent) {
  ...
  if (!shell) {
    message.reply('Sorry, we are not able to find your default shell.\nPlease set `process.env.SHELL`.');
    return;
  }
  ...
  res.on('error', (err) => {
    return message.reply(`Error occurred while spawning process\n${codeBlock.construct(err.toString(), 'sh')}`);
  });
  res.on('close', (code) => {
...
function kill(res, signal) {
  if (process.platform === 'win32') {
    return child.exec(`powershell -File "..\\utils\\KillChildrenProcess.ps1" ${res.pid}`, { cwd: __dirname });
